...
uint32_t calc_CRC(uint32_t addr, uint32_t length)
{
    ...
    {
        MAP_CRC32_set8BitDataReversed(__data20_read_char(addrIndex),
        CRC32_MODE);
    }
...
uint32_t foe_write_flash(foe_file_cfg_t *writefile_cfg, uint8_t *data,
                         size_t length)
{
...
uint32_t foe_read_flash(foe_file_cfg_t *writefile_cfg, uint8_t *data,
                        size_t length)
{
    int i;
    ...
    ui32FlashAddr = writefile_cfg->address_offset
            + writefile_cfg->dest_start_address;
    DPRINT("%s FlashAddr 0x%04X with %d bytes\n", __FUNCTION__, ui32FlashAddr,
           bytestoread);
